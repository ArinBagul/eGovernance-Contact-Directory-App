...
const ContactCard = ({ data }) => {
  ...
  useEffect(() => {
    if (!data.id) {
      console.log("Data ID is empty. Cannot fetch data.");
      return;
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (value !== null) {
          setFavorites(true); // Set favorites to true if item exists in AsyncStorage
        }
      } catch (error) {
  ...
  const handleFav = () => {
    if (!favorites) { // Add to favorites
      const key = String(data.id);
      try {
        const value = JSON.stringify(data); // Stringify the data
        AsyncStorage.setItem(key, value); // Store the stringified data
        setFavorites(true);
      } catch (error) {
      ...
      const key = String(data.id);
      try {
        AsyncStorage.removeItem(key); // Remove item from AsyncStorage
        setFavorites(false);
      } catch (error) {
    ...
    <View style={style.cardContainer}>
      ...
      <View style={style.bottomContainer}>
        ...
        <View style={style.iconsContainer}>
          <TouchableOpacity style={style.heartBtn} onPress={()=>{handleFav()}}>
            {favorites ? (
              <CustomTabBarIcon
                icon={heatFilled}
                size={27}
